...
<body  class="easyui-layout">   
		...
		<script type="text/javascript">
			...
			//查询方法
			function searchFun(){
				productdg.datagrid('load',base.serializeObject($('#searchForm')));
			}
			//清空查询表单
			function clearFun(){
			...
			function insertFun(){
				addProductDialog =$("<div/>").dialog({
								    ...
								    closed: false,    
								    cache: false,    
								    href: '${pageContext.request.contextPath}/product/toAddProductPage',    
								    modal: true,
								    buttons : [ {
										text : '添加',
										handler : function() {
											$('#addProductForm').form('submit', {   
											    ...
											    success: function(data){ 
											        ...
											        if (json.status==0){
											        	...
											        	//$('#dg').datagrid('reload');
											        	//将新增的数据直接添加到数据列表中
											        	productdg.datagrid('insertRow',{index:0,row:json.array});
											        	//消息提示 
									...
									onClose:function(){
										//在窗口关闭之后触发
										addProductDialog.dialog('destroy');
									}  
							});
			}
			 ...
			 * @returns {} 
			 */
			var editProductDialog;
			function editFun(){
				//首先判断是否有勾选记录
				var rows = productdg.datagrid('getChecked');
				
				//获取编辑行的索引值，用于编辑成功后直接更新该行的数据
				var rowindex = productdg.datagrid('getRowIndex',rows[0]);
				if(rows.length==1){
					editProductDialog =$("<div/>").dialog({
								title: '编辑',    
							    width: 1000,    
							    height:600,    
							    closed: false,    
							    cache: false,    
							    href: '${pageContext.request.contextPath}/product/toEditProductPage',    
							    modal: true,
							    buttons : [ {
									text : '编辑',
									handler : function() {
										$('#editProductForm').form('submit', {   
											    ...
											    success: function(data){ 
											        ...
											        if (json.status==0){
											        	...
											        	//$('#dg').datagrid('reload');
											        	//将编辑的数据直接更新数据列表中对应的列
											        	productdg.datagrid('updateRow',{index:rowindex,row:json.array});
								...
								onLoad:function(){
									//加载编辑窗口的时候，给表单赋值
									$.post('${pageContext.request.contextPath}/product/findProductById',{id:rows[0].id},function(data){
										var json=$.parseJSON(data); 
										if(json.status==0){
											$('#editproductForm').form('load',json.array);
											$('#logo').attr('src','${pageContext.request.contextPath}/product/findImgById?id='+rows[0].id);
								...
								onClose:function(){
									//在窗口关闭之后触发
									editProductDialog.dialog('destroy');
								}
